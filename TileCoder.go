...
// TODO: make a tiling struct that will encode observation(s) itself
...
func New(minDims, maxDims mat.Vector, bins [][]int,
	...
	return &TileCoder{numTilings, minDims, offsets, bins, binLengths, seed,
		includeBias, sync.WaitGroup{}, indices, vIndices}
}
...
// ! returns a matrix where each column is the non-zero indices for element i in the batch
...
func (t *TileCoder) encodeBatchWithTiling(b *mat.Dense,
	...
	for i := len(t.bins[tiling]) - 1; i > -1; i-- {
		...
		// Offset the tiling
		data.AddScaledVec(data, t.offsets[tiling].At(0, i), ones)
		...
		matutils.VecFloor(data, t.binLengths[tiling][i])

		// If out-of-bounds, use the last tile
		matutils.VecClip(data, 0.0, float64(t.bins[tiling][i]-1))
		...
		if i == len(t.bins[tiling])-1 {
			...
			index.AddScaledVec(index, float64(t.bins[tiling][i+1]), data)
		}
	}
...
func (t *TileCoder) EncodeBatch(b *mat.Dense) *mat.Dense {
	...
	for row := 0; row < t.VecLength(); row++ {
		colIndices := indices.RawRowView(row)
		for c := range colIndices {
			tileCoded.Set(row, c, 1.0)
		}
	}
